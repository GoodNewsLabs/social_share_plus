...
public class ShareUtil{
    ...
    func shareToTwitter(args : [String: Any?],result: @escaping FlutterResult) {
        ...
        guard let composeCtl = SLComposeViewController(forServiceType: SLServiceTypeTwitter) else {
            result(ERROR)
            return
        }
        ...
        if #unavailable(iOS 16) {
            if let text = title {
                composeCtl.add(URL(string: text))
            }
        }
        
        if let imagePaths = images {
            for imagePath in imagePaths {
                if let image = UIImage(contentsOfFile: imagePath) {
                    composeCtl.add(image)
                }
            }
        }
        ...
        // Use formSheet style - matches system share sheet behavior
        // This gives a centered card on iPad and bottom sheet on iPhone
        composeCtl.modalPresentationStyle = .formSheet
        
        // Use coverVertical for smooth slide up/down animation
        composeCtl.modalTransitionStyle = .coverVertical
        
        // Set the navigation bar to not be translucent to fix overlap issue
        if let navigationController = composeCtl.children.first as? UINavigationController {
            navigationController.navigationBar.isTranslucent = false
        }
        
        // Pre-load the view to avoid lag during presentation
        _ = composeCtl.view
        
        // Present with smooth animation on main thread
